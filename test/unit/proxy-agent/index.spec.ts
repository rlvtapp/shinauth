/**
 * Unit Test: Verify undici ProxyAgent works with shinauth OAuth flows
 *
 * This test demonstrates that when you set a global dispatcher with ProxyAgent,
 * all fetch() calls made by shinauth during OAuth flows are routed through it.
 ...
 * Related issue: https://github.com/rlvtapp/shinauth/issues/7396
 */
...
describe("ProxyAgent with shinauth OAuth", () => {
	...
	it("should route GitHub OAuth requests through global dispatcher", async () => {
		...
		// Create shinauth instance with GitHub OAuth
		...
		// Verify that shinauth made requests through our global dispatcher (proxy)
	...
	it("should route Google OAuth requests through global dispatcher", async () => {
		...
		// Create shinauth instance with Google OAuth
		const { client } = await getTestInstance({
			secret: DEFAULT_SECRET,
			socialProviders: {
				google: {
