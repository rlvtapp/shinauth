...
/**
 * @see https://github.com/rlvtapp/shinauth/issues/9233
 *
 ...
 * When HMAC verification fails on the old `session_data` cookie, Shinauth
...
describe("cookieCache HMAC verification failure fallback", async () => {
	...
	it("should fall through to session_token DB validation when session_data HMAC fails", async () => {
		...
		const sessionToken = cookies.get("shinauth.session_token");
		...
		tampered.set(
			"cookie",
			`shinauth.session_data=${invalidSessionData}; shinauth.session_token=${sessionToken}`,
		);
	...
	it("should expire malformed compact session_data and fall through to session_token DB validation", async () => {
		...
		tampered.set(
			"cookie",
			`shinauth.session_data=${btoa("not-json")}; shinauth.session_token=${sessionToken}`,
		);
		let setCookieHeader = "";
		const session = await client.getSession({
			fetchOptions: {
		...
		expect(session.data).not.toBeNull();
		expect(session.data?.user.email).toBe(testUser.email);
		expect(setCookieHeader).toContain("shinauth.session_data=");
	...
	it("should still return null when both session_data and session_token are invalid", async () => {
		...
		tampered.set(
			"cookie",
			`shinauth.session_data=${invalidSessionData}; shinauth.session_token=invalid-token.invalid-signature`,
		);
	...
	it("should work with JWT strategy when HMAC verification fails", async () => {
		...
		tampered.set(
			"cookie",
			`shinauth.session_data=invalid.jwt.token; shinauth.session_token=${sessionToken}`,
		);
	...
	it("should work with JWE strategy when decryption fails", async () => {
		...
		tampered.set(
			"cookie",
			`shinauth.session_data=invalid.jwe.token.here.test; shinauth.session_token=${sessionToken}`,
		);
	...
	it("should handle cross-subdomain cookie migration without silent logout", async () => {
		...
		const cookies = parseCookies(cookieStr);
		const sessionToken =
			cookies.get("__Secure-shinauth.session_token") ||
			cookies.get("shinauth.session_token");
		expect(sessionToken).toBeDefined();
		...
		migrationHeaders.set(
			"cookie",
			`__Secure-shinauth.session_data=${staleSessionData}; __Secure-shinauth.session_token=${sessionToken}`,
		);
	...
	it("should work during cookieCache window even with stale session_data", async () => {
		...
		migrationHeaders.set(
			"cookie",
			`shinauth.session_data=${staleSessionData}; shinauth.session_token=${sessionToken}`,
		);
