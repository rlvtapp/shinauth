...
describe("account", async () => {
	...
	/**
	 * @see https://github.com/rlvtapp/shinauth/issues/8345
	 */
	...
	/**
	 * @see https://github.com/rlvtapp/shinauth/issues/8350
	 */
	...
	/**
	 * @see https://github.com/rlvtapp/shinauth/issues/9502
	 */
	...
	/**
	 * @see https://github.com/rlvtapp/shinauth/issues/2351
	 */
	...
	/**
	 * @see https://github.com/rlvtapp/shinauth/issues/8562
	 */
	...
	it("should encrypt account cookie payload", async () => {
		...
		await client.$fetch("/callback/google", {
			...
			async onError(context) {
				...
				await expect(
					symmetricDecodeJWT(accountData!, ctx.secret, "shinauth-account"),
				).resolves.toMatchObject({
					accessToken: "test",
					refreshToken: "test",
					providerId: "google",
...
/**
 * @see https://github.com/rlvtapp/shinauth/issues/9732
 */
			...
			const linkAccountRes = await client.linkSocial(
				...
				},
				{
					onSuccess(context) {
						...
						const state = cookies.get("shinauth.state")?.value;
						headers.set(
							"cookie",
							`${headers.get("cookie") || ""}; shinauth.state=${state}`,
						);
					},
				},
			);
...
/**
 * @see https://github.com/rlvtapp/shinauth/issues/9967
 */
...
describe("account resolution in stateless mode", async () => {
	...
	/**
	 * @see https://github.com/rlvtapp/shinauth/issues/9978
	 */
