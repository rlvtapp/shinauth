...
describe("session", async () => {
	...
	/**
   @see https://github.com/rlvtapp/shinauth/issues/9609
   */
		...
		await client.getSession({
			fetchOptions: {
				...
				onSuccess(context) {
					...
					refreshedMaxAge = parsed.get("shinauth.session_token")?.[
						"max-age"
					];
				},
			},
...
describe("cookie cache with JWT strategy backed by JWKS", async () => {
	...
	it("should ignore a JWKS cookie cache from a different session token", async () => {
		...
		mixedHeaders.set(
			"cookie",
			`shinauth.session_data=${firstCookieCache}; shinauth.session_token=${secondSessionCookie}`,
		);
	...
	it("should not allow tampering with the cookie in JWKS mode", async () => {
		...
		headers.set(
			"cookie",
			`shinauth.session_data=${tamperedToken}; shinauth.session_token=${sessionCookie}`,
		);
		const res = await client.getSession({
			fetchOptions: {
				headers,
...
describe("cookie cache refreshCache", async () => {
	...
	/**
	 * @see https://github.com/rlvtapp/shinauth/pull/8817
	 */
	...
	/**
	 * @see https://github.com/rlvtapp/shinauth/issues/8763
	 */
...
describe("get-session cache headers", async () => {
	/**
	 * @see https://github.com/rlvtapp/shinauth/issues/10217
	 */
	...
	/**
	 * @see https://github.com/rlvtapp/shinauth/pull/10222
	 */
