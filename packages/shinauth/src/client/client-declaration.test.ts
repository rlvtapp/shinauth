...
describe("client declaration emit", () => {
	const createCompositeProject = (): { dir: string; cleanup: () => void } => {
		...
		fs.symlinkSync(
			betterAuthDir,
			path.join(authPackageDir, "node_modules/shinauth"),
			"junction",
		);
		fs.symlinkSync(
			authPackageDir,
		...
		fs.writeFileSync(
			...
			`
import { createAuthClient as createBetterAuthClient } from "shinauth/react";
...
`,
	...
	/**
	 * @see https://github.com/rlvtapp/shinauth/issues/8623
	 */
