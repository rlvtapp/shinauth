...
import { getBetterAuthVersion } from "@shinauth/core/context";
import { getAuthTables } from "@shinauth/core/db";
import type { DBAdapter } from "@shinauth/core/db/adapter";
import { createLogger, env, isProduction, isTest } from "@shinauth/core/env";
import { BetterAuthError } from "@shinauth/core/error";
import type { OAuthProvider } from "@shinauth/core/oauth2";
import type { SocialProviders } from "@shinauth/core/social-providers";
import { socialProviders } from "@shinauth/core/social-providers";
import { generateId } from "@shinauth/core/utils/id";
import { findInvalidTrustedProxies } from "@shinauth/core/utils/ip";
import { createTelemetry } from "@shinauth/telemetry";
...
Most of the features of Shinauth will not work correctly.`,
	...
	if (cookieCacheJwtSigningKey === "jwt-plugin") {
		...
		if (gracePeriod !== undefined && cookieMaxAge > gracePeriod) {
			logger.warn(
				`[shinauth] \`session.cookieCache.maxAge\` (${cookieMaxAge}s) exceeds the JWT plugin JWKS grace period (${gracePeriod}s). Rotated keys may stop verifying cookie-cache JWTs before the cookie expires.`,
			);
		}
	}
