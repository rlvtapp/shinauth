...
export const initMinimal = async (options: BetterAuthOptions) => {
	const adapter = await getBaseAdapter(options, async () => {
		throw new BetterAuthError(
			"Direct database connection requires Kysely. Please use `shinauth` instead of `shinauth/minimal`, or provide an adapter (drizzleAdapter, prismaAdapter, etc.)",
		);
	});
	...
	ctx.runMigrations = async function () {
		throw new BetterAuthError(
			"Migrations are not supported in 'shinauth/minimal'. Please use 'shinauth' for migration support.",
		);
	};
