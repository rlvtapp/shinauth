...
export async function getMigrations(config: BetterAuthOptions) {
	...
	function getType(field: DBFieldAttribute, fieldName: string) {
		...
		if (!(type in typeMap)) {
			throw new Error(
				`Unsupported field type '${String(type)}' for field '${fieldName}'. Allowed types are: string, number, boolean, date, string[], number[]. If you need to store structured data, store it as a JSON string (type: "string") or split it into primitive fields. See https://shinauth.com/docs/advanced/schema#additional-fields`,
			);
		}
