...
import type { DBFieldAttribute, DBFieldType } from "@shinauth/core/db";
import { getAuthTables } from "@shinauth/core/db";
import { initGetFieldName, initGetModelName } from "@shinauth/core/db/adapter";
import { createLogger } from "@shinauth/core/env";
import type { KyselyDatabaseType } from "@shinauth/kysely-adapter";
import { createKyselyAdapter } from "@shinauth/kysely-adapter";
import type {
...
export async function getMigrations(config: BetterAuthOptions) {
	...
	function getType(field: DBFieldAttribute, fieldName: string) {
		...
		if (!(type in typeMap)) {
			throw new Error(
				`Unsupported field type '${String(type)}' for field '${fieldName}'. Allowed types are: string, number, boolean, date, string[], number[]. If you need to store structured data, store it as a JSON string (type: "string") or split it into primitive fields. See https://github.com/rlvtapp/shinauth`,
			);
		}
