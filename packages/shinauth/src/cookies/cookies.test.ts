...
import { createHMAC } from "@shinauth/utils/hmac";
...
const COOKIE_CACHE_JWT_TYPE = "shinauth.session-cache+jwt";
const COOKIE_CACHE_JWT_AUDIENCE = "shinauth:session-cache";
		...
		await client.signIn.email(
			...
			},
			{
				onResponse(context) {
					...
					expect(setCookie).toContain("shinauth");
				},
			},
		);
	});
...
/**
 * @see https://github.com/shinauth/better-call/issues/54
 * @see https://github.com/rlvtapp/shinauth/pull/8089
 */
...
describe("getSessionCookie", async () => {
	...
	it("should work with cross-realm Headers-like object (instanceof fails)", async () => {
		...
		// See: https://github.com/rlvtapp/shinauth/pull/1838
	...
	describe.each([".", "-"])("with '%s' separator", (separator) => {
		describe.each([
			...
			it.each([
				...
				[{ cookieName: "my_token" }, "shinauth", "my_token"],
				[
...
describe("Cookie Cache Field Filtering", () => {
	...
	it("should require JWKS input for helper verification in JWKS mode", async () => {
		const headers = new Headers();
		headers.set("cookie", "shinauth.session_data=token");
	...
	it("should reject JWT plugin tokens that are not cookie-cache JWTs", async () => {
		...
		const headers = new Headers();
		headers.set("cookie", `shinauth.session_data=${token.token}`);
		const request = new Request("https://example.com/api/auth/session", {
			headers,
		});
...
describe("Cookie Chunking", () => {
	...
	/**
	 * @see https://github.com/rlvtapp/shinauth/issues/8585
	 */
	it("should chunk session cache when attributes push the line over the limit", async () => {
		...
		const longPrefix = "shinauth-" + "x".repeat(80);
		const { client } = await getTestInstance({
			secret: "shinauth.secret",
	...
	it("skips the session cache and warns when the prefix alone overflows", async () => {
		...
		const longPrefix = "shinauth-" + "x".repeat(4100);
		const { client, auth, cookieSetter } = await getTestInstance({
			secret: "shinauth.secret",
			advanced: { cookiePrefix: longPrefix },
			logger: {
...
/**
 * @see https://github.com/rlvtapp/shinauth/issues/9465
 */
describe("Cookie header without whitespace after semicolon", () => {
	it("parseCookies returns each pair when separator is `;` only", () => {
		...
		expect(parsed.get("shinauth.session_data")).toBe(
			"session-data.signature",
		);
	});
	...
	it("getSessionCookie finds the session cookie when separator is `;` only", () => {
		...
		headers.set(
			"cookie",
			"preference=dark;shinauth.session_token=token-123",
		);
		const request = new Request("https://example.com/api/auth/session", {
			headers,
		});
	...
	it("getChunkedCookie reconstructs chunks across `;`-only separators", () => {
		...
		expect(getChunkedCookie(ctx, "shinauth.session_data")).toBe(
			"chunkAchunkB",
		);
	});
});
