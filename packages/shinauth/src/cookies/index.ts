...
import { env, isProduction } from "@shinauth/core/env";
...
import { binary } from "@shinauth/utils/binary";
...
export function createCookieGetter(options: BetterAuthOptions) {
	...
	function createCookie(
		...
		const prefix = options.advanced?.cookiePrefix || "shinauth";
		const name =
...
export const getSessionCookie = (
	...
	const { cookieName = "session_token", cookiePrefix = "shinauth" } =
		config || {};
...
>(
	...
	const { cookieName = "session_data", cookiePrefix = "shinauth" } =
		config || {};
	const name =
