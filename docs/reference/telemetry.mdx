---
title: "Telemetry"
description: Shinauth now collects anonymous telemetry data about general usage.
---

Shinauth collects anonymous usage data to help us improve the project. This is optional, transparent, and disabled by default.

## Why is telemetry collected?

Since v1.3.5, Shinauth collects anonymous telemetry data about general usage if enabled.

Telemetry data helps us understand how Shinauth is being used across different environments so we can improve performance, prioritize features, and fix issues more effectively. It guides our decisions on performance optimizations, feature development, and bug fixes. All data is collected completely anonymously and with privacy in mind, and users can opt out at any time. We strive to keep what we collect as transparent as possible.
...
* **Redacted auth config snapshot**: A minimized, privacy‑preserving view of your `shinAuth` options produced by `getTelemetryAuthConfig`.
...
All collected data is fully anonymous and only useful in aggregate. It cannot be traced back to any individual source and is accessible only to a small group of core Shinauth maintainers to guide roadmap decisions.

* **No PII or secrets**: We do not collect emails, usernames, tokens, secrets, client IDs, client secrets, or database URLs.
* **No full config**: We never send your full `shinAuth` configuration. Instead we send a reduced, redacted snapshot of non‑sensitive toggles and counts.
* **Redaction by design**: See [detect-auth-config.ts](https://github.com/rlvtapp/shinauth/blob/main/packages/telemetry/src/detectors/detect-auth-config.ts) in the Shinauth source for the exact shape of what is included. It purposely converts sensitive values to booleans, counts, or generic identifiers.
...
* On `shinAuth` initialization (`type: "init"`).
