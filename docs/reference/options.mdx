---
title: "Options"
description: Shinauth configuration options reference.
---

List of all the available options for configuring Shinauth. See [Shinauth Options](https://github.com/rlvtapp/shinauth/blob/main/packages/core/src/types/init-options.ts).

## `appName`

The name of your application. Defaults to `"Shinauth"`.

It's used as a display name in contexts where your app needs to be identified — for example, when users set up two-factor authentication with an authenticator app (like Google Authenticator), `appName` appears as the issuer name in the TOTP entry. This can be overridden per-plugin (e.g. the [2FA plugin's `issuer` option](/plugins/2fa#issuer)).
...
Base URL for Shinauth. This is typically the root URL where your application server is hosted. You can configure it as either:
...
Use the object form when your app can be reached through multiple domains, such as preview deployments, branch environments, or multiple production hosts. Shinauth derives the host from the incoming request, validates it against `allowedHosts`, and constructs the request-specific base URL from that match.
...
For deployment patterns and end-to-end examples, see the [Dynamic Base URL guide](/guides/dynamic-base-url).

## `basePath`

Base path for Shinauth. This is typically the path where the Shinauth routes are mounted. It will be overridden if there is a path component within `baseURL`.
...
By default, Shinauth trusts the base URL of your app (i.e. [`baseURL`](#baseurl)). You can specify additional trusted origins via this option. Values can be a static array of origins, a function that returns origins dynamically, or wildcard patterns to match multiple domains.
...
By default, Shinauth will look for the following environment variables:

* `process.env.BETTER_AUTH_SECRET`
* `process.env.AUTH_SECRET`

If none of these environment variables are set, it will default to `"shinauth-secret-12345678901234567890"`. In production, if it's not set, it will throw an error.
...
Database configuration for Shinauth.
...
Shinauth supports various database configurations including [PostgreSQL](/adapters/postgresql), [MySQL](/adapters/mysql), and [SQLite](/adapters/sqlite).

Read more about [databases](/concepts/database).
...
Read more about [secondary storage](/concepts/database#secondary-storage).
...
* `customSyntheticUser`: Function to build a custom synthetic user for email enumeration protection. Use when plugins add fields to the user table. See [email enumeration protection](/authentication/email-password#plugins-that-add-user-fields).
...
The account cookie has a five-minute max age and is refreshed when Shinauth writes updated provider account data, for example after `getAccessToken` or `/refresh-token` refreshes provider tokens. When this happens in server-side code, forward the returned `Set-Cookie` header to the browser so the refreshed account cookie is retained.

Shinauth chunks oversized account cookies, but browsers and proxies can still enforce total cookie or header limits. Prefer database-backed account storage for providers that issue large JWTs or for production flows that need durable token storage.

When this option is enabled, you can read the decrypted account cookie in hooks or middleware using `getAccountCookie` from `shinauth/cookies`:

```ts
import { createAuthMiddleware } from "shinauth/api";
import { getAccountCookie } from "shinauth/cookies";
...
If `secondaryStorage` is configured, verification records are stored there by default. That behavior applies to flows that use Shinauth's shared verification layer, including OTP and magic-link style flows.
...
* `generateId`: Controls how record IDs are generated. Accepts a custom function, `false`, `"serial"`, or `"uuid"` (default: [random base62 string](https://github.com/rlvtapp/shinauth/blob/main/packages/core/src/utils/id.ts)). See the [Database documentation](/concepts/database#id-generation) for more info.
...
Logger configuration for Shinauth.
...
The logger configuration allows you to customize how Shinauth handles logging. It supports the following options:
...
* `customizeDefaultErrorPage`: Configure the default error page provided by Shinauth. Start your dev server and go to `/api/auth/error` to see the error page.
...
For more details and examples, see the [Hooks documentation](/concepts/hooks).
...
Enable or disable Shinauth's telemetry collection. (default: `false`)
