...
description: Integrate Shinauth with Next.js.
---

Shinauth can be easily integrated with Next.js. Before you start, make sure you have a Shinauth instance configured. If you haven't done that yet, check out the [installation](/installation).
...
**Note:**
  ...
  You can change the path on your shinauth configuration but it's recommended to keep it as `/api/auth/[...all]`
...
The `api` object exported from the auth instance contains all the actions that you can perform on the server. Every endpoint made inside Shinauth is invocable as a function. Including plugins endpoints.
...
**Warning:**
As RSCs cannot set cookies, the [cookie cache](/concepts/session-management#cookie-cache) will not be refreshed until the server is interacted with from the client via Server Actions or Route Handlers.
...
**Note:**
  ...
  **Migration from middleware:** Rename `middleware.ts` → `proxy.ts` and `middleware` → `proxy` function. All Shinauth methods work identically.
...
```ts title="middleware.ts"
import { getCookieCache } from "shinauth/cookies";
...
Shinauth is fully compatible with Next.js 16. The main change is that "middleware" is now called "proxy". See the [Auth Protection](#auth-protection) section above for Next.js 16+ proxy examples.
...
All Shinauth methods work identically. See the [Next.js migration guide](https://nextjs.org/docs/app/api-reference/file-conventions/proxy#migration-to-proxy) for details.
