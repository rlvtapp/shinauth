...
description: Integrate Shinauth with Electron.
---

Electron is a popular framework for building cross-platform desktop applications using web technologies.

Shinauth can be integrated into Electron apps to provide secure authentication flows, leveraging the system browser.

## Installation

### Configure a Shinauth front- & back-end

Before integrating with Electron, ensure you have a Shinauth server and client set up.

To get started, check out our [installation](/installation) guide for setting up Shinauth.

### Install the required packages

Install the Shinauth server package and the Electron integration package in your server, Electron app, and web client projects.

**In each project, run:**

```bash
shinauth @shinauth/electron
```
...
### Add the Electron plugin to your Shinauth server
...
export const authClient = createAuthClient({
  baseURL: "http://localhost:8081", // Base URL of your Shinauth frontend
  plugins: [
    electronClient({
...
To enable this, you need to add your app's protocol scheme to the `trustedOrigins` on your **Shinauth server**.
...
Then, update your Shinauth config to include the scheme in `trustedOrigins`:
...
To run your process in `sandbox` mode, you need to ensure that `@shinauth/electron` is bundled into the preload script.
Import only `@shinauth/electron/preload` inside the preload script with tree-shaking enabled, to avoid bundling unwanted dependencies.
...
If your config uses a list of externals instead, do **not** add `@shinauth/electron` to that list so it gets bundled.
...
To handle already signed-in users, you can use the `electron.transferUser` method on the client and `electronTransferUser` on the server to transfer the user to the Electron app. [See Example](https://github.com/rlvtapp/shinauth/blob/main/demo/nextjs/app/\(auth\)/sign-in/_components/electron.tsx)
...
{/* or ![Screenshot](user-image://<user-id>) */}
```
...
To access avatars for users other than the current one, you need to enable the [Admin](/plugins/admin) plugin.
...
The prefix to use for cookies set by the plugin. (defaults to `shinauth`)
...
Prefix for local storage keys. (defaults to `shinauth`)
...
Prefix(es) for server cookie names to filter. (defaults to `shinauth`)

This is used to identify which cookies belong to shinauth to
...
```ts title="electron/auth-client.ts"
electronClient({
  cookiePrefix: ["shinauth", "my-app"],
});
```

#### `channelPrefix?`

Channel prefix for IPC bridges. (defaults to `shinauth`)
