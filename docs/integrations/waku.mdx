...
description: Integrate Shinauth with Waku.
---

Shinauth can be easily integrated with Waku. Before you start, make sure you have a Shinauth instance configured. If you haven't done that yet, check out the [installation](/installation).

## Create auth instance

Create a file named `auth.ts` in your application. Import Shinauth and create your instance.
...
**Note:**
  ...
  You can change the path on your shinauth configuration but it's recommended to keep it as `src/pages/_api/api/auth/[...route].ts`
...
**Warning:**
RSCs that run after the response has started streaming cannot set cookies. The [cookie cache](/concepts/session-management#cookie-cache) will not be refreshed until the server is interacted with from the client via Server Actions or Route Handlers.
...
You can use the `getSessionCookie` helper from Shinauth for this purpose:
