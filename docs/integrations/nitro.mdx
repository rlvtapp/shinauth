...
description: Integrate Shinauth with Nitro.
---

Shinauth can be integrated with your [Nitro Application](https://nitro.build/) (an open source framework to build web servers).

This guide aims to help you integrate Shinauth with your Nitro application in a few simple steps.
...
### Install & Configure Shinauth

Follow steps 1 & 2 from the [installation guide](/installation) to install Shinauth in your Nitro application & set up the environment variables.

Once that is done, create your Shinauth instance within the `server/utils/auth.ts` file.
...
Use the Shinauth CLI to update your Prisma schema with the required models by running the following command:

```bash title="Terminal"
pnpm dlx @shinauth/cli generate --config server/utils/auth.ts
```

**Note:**

The `--config` flag is used to specify the path to the file where you have created your Shinauth instance.
...
You can now mount the Shinauth handler in your Nitro application. You can do this by adding the following code to your `server/routes/api/auth/[...all].ts` file:
...
See an [example Nitro application integrated with Shinauth & Prisma](https://github.com/BayBreezy/nitrojs-shinauth-prisma).
