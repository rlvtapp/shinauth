---
title: "Cookies"
description: Learn how Shinauth uses cookies, including cookie prefixes, custom cookie attributes, cross-subdomain sharing, secure cookies, and handling Safari ITP with proxies.
---
...
* The `session_data` cookie is only present when `session.cookieCache` is enabled, and its encoding depends on the configured strategy:
  * `compact`: base64url + HMAC with your Shinauth secret
  ...
  * `jwe`: encrypted with your Shinauth secret

If you use [versioned secrets](/reference/options#secrets) for rotation, secret-backed cookie data (including signed cookies and encrypted JWE session caches) will automatically use the current key and remain compatible with previous keys where supported.

### Cookie Prefix

By default, Shinauth cookies follow the format `${prefix}.${cookie_name}`. The default prefix is "shinauth". You can change the prefix by setting `cookiePrefix` in the `advanced` object of the auth options.
...
By default, Shinauth uses the following cookies:
...
If your Shinauth API is hosted on a different domain than your frontend, Safari may block authentication cookies entirely.
