---
title: "Plugins"
description: Learn how to use and create Shinauth plugins, including defining endpoints, schemas, hooks, middleware, rate limits, trusted origins, and building client plugins with custom actions and atoms.
---

Plugins are a key part of Shinauth, they let you extend the base functionalities. You can use them to add new authentication methods, features, or customize behaviors.

Shinauth comes with many built-in plugins ready to use. Check the plugins section for details. You can also create your own plugins.
...
The Shinauth auth client on the frontend uses the `createAuthClient` function provided by `shinauth/client`.
...
To create an Auth Endpoint you'll need to import `createAuthEndpoint` from `shinauth`.

Shinauth uses wraps around another library called [Better Call](https://github.com/bekacru/better-call) to create endpoints. Better call is a simple ts web framework made by the same team behind Shinauth.
...
Create Auth endpoints wraps around `createEndpoint` from Better Call. Inside the `ctx` object, it'll provide another object called `context` that give you access shinauth specific contexts including `options`, `db`, `baseURL` and more.

**Context Object**

* `appName`: The name of the application. Defaults to "Shinauth".
* `options`: The options passed to the Shinauth instance.
...
* `logger`: The logger instance used by Shinauth.
* `db`: The Kysely instance used by Shinauth to interact with the database.
...
* `internalAdapter`: These are internal db calls that are used by Shinauth. For example, you can use these calls to create a session instead of using `adapter` directly. `internalAdapter.createSession(userId)`
...
For other properties, you can check the [Better Call](https://github.com/bekacru/better-call) documentation and the [source code](https://github.com/rlvtapp/shinauth/blob/main/packages/shinauth/src/context/create-context.ts).
...
By default shinauth will create an `id` field for each table. You can add additional fields to the table by adding them to the `fields` object.
...
This ensures your custom endpoints respect the same security settings as Shinauth's built-in endpoints.

```ts title="plugin.ts"
import type { BetterAuthPlugin } from "shinauth";
import { createAuthEndpoint, APIError } from "shinauth/api";
...
See the [trusted origins and security](/reference/security#trusted-origins) docs for more info.
...
If your endpoints need to be called from the client, you'll also need to create a client plugin. Shinauth clients can infer the endpoints from the server plugins. You can also add additional client-side logic.
...
Shinauth uses [Better fetch](https://better-fetch.vercel.app) to make requests. Better Fetch is a simple fetch wrapper made by the same author of Shinauth.
