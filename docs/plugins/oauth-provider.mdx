...
description: A Shinauth plugin that enables your auth server to serve as an OAuth 2.1 provider.
---
...
* **JWT Plugin compatibility**: required by default with an option to disable
  ...
  * **JWKS Verifiable**: verify tokens remotely at the [`/jwks`](/plugins/jwt#verifying-the-token) endpoint
...
Shinauth serves the OAuth Authorization Server metadata and OpenID Connect discovery metadata from the auth handler automatically. If your framework only forwards requests under a catch-all auth route, make sure the issuer metadata URLs reach `auth.handler`.
...
To enable open client registration without a Shinauth session, additionally set `allowUnauthenticatedClientRegistration: true` in your auth config. Public clients are registered with `token_endpoint_auth_method: "none"`. Confidential clients receive a one-time `client_secret` in the registration response.

**Note:**

For MCP public-client identity, [Client ID Metadata Documents](https://github.com/modelcontextprotocol/modelcontextprotocol/issues/991) are [available](/plugins/cimd).
...
The authorization server accepts requests without `state` for compatibility with OAuth and OpenID Connect, and echoes `state` back when it is provided. Shinauth's client helpers generate and validate `state` for you.
...
Shinauth supports Demonstrating Proof of Possession (DPoP) as defined by [RFC 9449](https://datatracker.ietf.org/doc/html/rfc9449). A client can ask for DPoP-bound tokens by registering with `dpop_bound_access_tokens: true`, by sending `dpop_jkt` on the authorization request, or by requesting a resource configured with `dpopBoundAccessTokensRequired: true`.
...
The OAuth Provider plugin serves this endpoint automatically from the Shinauth handler. If you do not set a custom issuer, the issuer path is your basePath, such as `/api/auth`.
...
import { oauthProviderOpenIdConfigMetadata } from "@shinauth/oauth-provider";
import { auth } from "@/lib/auth";
...
The OAuth Provider plugin serves both path-prefixed issuer aliases automatically from the Shinauth handler:
...
import { oauthProviderAuthServerMetadata } from "@shinauth/oauth-provider";
import { auth } from "@/lib/auth";
...
Verification can be performed using `verifyAccessTokenRequest` available through the `oauthProviderResourceClient` plugin or `shinauth/oauth2` package. This is the recommended resource-server API because it verifies the access token and, when the token is DPoP-bound, also verifies the request method, URL, `Authorization: DPoP` scheme, proof key, replay `jti`, and `ath` claim.

With `shinauth` package:
...
With the [Bearer](/plugins/bearer) plugin enabled, an `Authorization: Bearer` value that resolves to a valid user session is handled as that session, not as an initial access token.
...
```ts
import { consumeClientAssertion } from "@shinauth/oauth-provider";
...
OAuth Clients are tied to either a user or `reference_id` at registration and is immutable. If you are utilizing the [organization plugin](/plugins/organization), you must ensure that the [`activeOrganizationId`](/plugins/organization#active-organization) is set on your active session when you create new clients.
...
These rate limits only apply when Shinauth's global rate limiting is enabled. By default, rate limiting is only enabled in production. See [Rate Limiting](/concepts/rate-limit) for global configuration.
...
Setting an endpoint to `false` removes the OAuth Provider's stricter per-endpoint limit. The endpoint will still be subject to Shinauth's global rate limiting if enabled.
...
Shinauth advertises `acr_values_supported: ["0"]`, which is the unspecified authentication context. Custom ACR policies are not currently supported; authorization requests for other
...
If you use the [`cimd`](/plugins/cimd) plugin or `allowUnauthenticatedClientRegistration`, you must ensure that your API Server is a confidential client itself:
...
If you choose not to support the [`cimd`](/plugins/cimd) plugin or `allowUnauthenticatedClientRegistration` (and only `allowDynamicClientRegistration`), the MCP client (ie. ChatGPT, Anthropic, Gemini) would need to allow you to put in a public client\_id in their UI or at runtime while chatting with the AI.
