...
import { ssoClient } from "@shinauth/sso/client"
...
When you register an OIDC provider, Shinauth automatically fetches and validates the IdP's [OIDC discovery document](#oidc-discovery). Most endpoint fields are optional — see [OIDC Discovery](#oidc-discovery) for details on auto-discovered fields and possible registration errors.
...
Shinauth automatically fetches and validates the provider's [OpenID Connect Discovery Document](https://openid.net/specs/openid-connect-discovery-1_0.html) from:
...
type registerSSOProvider = {
    ...
    /**
     * Link this provider to an organization. Requires the organization plugin.
     */
    organizationId?: string = "org_acme_corp_id"
    /**
     * When orgAccess is enabled, require users to access the linked organization
     * from an SSO-authenticated session.
     */
    requireSsoForOrgAccess?: boolean = true
    /**
...
Shinauth fills in the following fields by reading the IdP's discovery document (if not explicitly provided):
...
Both the discovery endpoint as well as any URL resolved through the discovery process are subject to your app's [`trustedOrigins`](/reference/security#trusted-origins) configuration.
...
See the [`trustedOrigins`](/reference/security#trusted-origins) docs for more information.

#### Redirecting OIDC endpoints

Shinauth does not follow redirects for server-side OIDC requests. Configure the final canonical URL for the discovery, token, userinfo, and JWKS endpoints.

If one of those endpoints returns a `3xx` response, registration or sign-in fails with `oidc_endpoint_redirect`. This keeps endpoint validation tied to the exact URL that Shinauth fetches.

#### Why Discovery Can Fail

Shinauth validates that the IdP's metadata is correct and complete **before** allowing registration. This prevents subtle runtime failures during sign-in or token validation.

**Note:**

Shinauth does **not** support implicit-only OIDC flows.
...
| `unsupported_token_auth_method` | The IdP only supports token auth methods that Shinauth doesn't support                                                              |
...
* Shinauth automatically performs OIDC discovery at registration time
...
await auth.api.registerSSOProvider({
    body: {
        ...
        requireSsoForOrgAccess: true, // Require SSO for this organization
        samlConfig: {
            // SAML configuration...
        },
    },
...
#### Requiring SSO for Organization Access

You can require members to access an organization through one of its linked SSO providers. This is useful when an organization wants its IdP policies, such as MFA, device checks, and offboarding, to control access to that organization.

First enable SSO-based organization access on the plugin:
...
When this is enabled, non-SSO sessions cannot access the linked organization through organization-scoped endpoints, and the organization is filtered from organization lists. A session created by signing in through any SSO provider linked to that organization can access it.

Set `orgAccess.requireSsoByDefault` to `true` if every linked SSO provider should require SSO unless the provider explicitly sets `requireSsoForOrgAccess: false`.
...
When your IdP rotates its signing certificate, list both the old and the new PEM under `idpMetadata.cert` as an array. Shinauth accepts SAML responses signed by either, so users keep signing in while the rotation completes.
...
Shinauth provides flexibility to support both:
...
Shinauth validates SAML cryptographic algorithms and warns about deprecated ones (SHA-1, RSA 1.5, 3DES) by default.
...
Shinauth enforces size limits on SAML payloads to protect against denial-of-service attacks via oversized XML.
...
When using a full URL, make sure it routes to your Shinauth instance.
...
* **Host:** `_shinauth-token-{your-provider-id}` (**Note:** An underscore is automatically prepended to follow DNS infrastructure subdomain conventions. The `shinauth-token` part can be customized through the `domainVerification.tokenPrefix` option)
...
Shinauth derives the ACS URL automatically from your `baseURL` and `providerId`, so no SP-side configuration is required.

Your IdP must include an `AudienceRestriction` for this SP. The accepted audience is the SP entity ID, or the explicit `audience` value in `samlConfig` when you need to match an IdP-specific SP identifier. Shinauth also checks bearer `Recipient` and response `Destination` values against this provider's ACS endpoints.
...
When `orgAccess.enabled` is `true`, the `ssoProvider` table also includes `requireSsoForOrgAccess`, and the session schema includes `authSource` and `authProviderId` so Shinauth can tell which provider created the current session.
...
Shinauth supports **IdP-initiated SSO flows**, where users access your application directly from their Identity Provider dashboard (e.g., Okta, Azure AD, OneLogin). This is common in enterprise environments where IT admins prefer centralized app access.

**How it works:**

1. User clicks your app icon in the IdP dashboard
2. IdP POSTs SAMLResponse to `/api/auth/sso/saml2/sp/acs/{providerId}`
3. Shinauth processes the assertion, creates a session, and redirects to your application
...
If you previously created a manual GET handler for the SAML callback route as a workaround, you can remove it after upgrading. Shinauth now handles GET requests automatically.

**Note:**

**Security:** Shinauth validates all redirect URLs to prevent open redirect attacks. Only relative paths (e.g., `/dashboard`) and URLs matching your configured `trustedOrigins` are allowed. Malicious URLs like `https://evil.com` or protocol-relative URLs (`//evil.com`) are automatically blocked.

For a detailed guide on setting up SAML SSO with examples for Okta and testing with DummyIDP, see our [SAML SSO with Okta](/guides/saml-sso-with-okta).
...
**orgAccess**: Enables SSO-based organization access controls. Set `orgAccess.enabled` to `true` to allow linked SSO providers to require SSO-authenticated sessions for organization access. Set `orgAccess.requireSsoByDefault` to make newly registered linked providers require SSO unless they explicitly opt out.
