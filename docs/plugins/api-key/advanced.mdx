...
Any time an endpoint in Shinauth is called that has a valid API key in the headers, you can automatically create a mock session to represent the user by enabling `enableSessionForAPIKeys` option.
...
For other endpoints/methods that don't use API keys, you should utilize Shinauth's [built-in rate-limiting](/concepts/rate-limit).
...
**Warning:**
  ...
  * Use `enableSessionForAPIKeys: true` and let Shinauth handle session creation automatically (recommended)
  * Or verify the API key once and reuse the validated result instead of calling both methods separately


You can refer to the rate-limit default configurations below in the [API Key plugin options](/plugins/api-key/reference#api-key-plugin-options).
