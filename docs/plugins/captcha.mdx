...
The **Captcha Plugin** integrates bot protection into your Shinauth system by adding captcha verification for key endpoints. This plugin ensures that only human users can perform actions like signing up, signing in, or resetting passwords. The following providers are currently supported:
...
This plugin works out of the box with [Email & Password](/authentication/email-password) authentication. To use it with other authentication methods, you will need to configure the [endpoints](/plugins/captcha#plugin-options) array in the plugin options.
