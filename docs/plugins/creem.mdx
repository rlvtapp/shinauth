...
* **Flexible Architecture** - Use Shinauth endpoints or direct server-side functions
...
Configure Shinauth with the Creem plugin:

```typescript
// lib/auth.ts
import { shinAuth } from "shinauth";
import { creem } from "@creem_io/shinauth";
...
import { creemClient } from "@creem_io/shinauth/client";
...
```typescript
// lib/auth-client.ts
import { createCreemAuthClient } from "@creem_io/shinauth/create-creem-auth-client";
import { creemClient } from "@creem_io/shinauth/client";
...
Depending on your database adapter, additional setup steps may be required. Refer to the [Shinauth adapter documentation](https://www.shinauth.com/docs/adapters/mysql) for details.
...
(`/api/auth` is the default Shinauth server path)
...
import { validateWebhookSignature } from "@creem_io/shinauth/server";
...
Use these utilities directly in Server Components, Server Actions, or API routes without going through Shinauth endpoints.
...
import {
  ...
  getDaysUntilRenewal,
  validateWebhookSignature,
} from "@creem_io/shinauth/server";
```

### Server Component Example

```typescript
import { checkSubscriptionAccess } from "@creem_io/shinauth/server";
...
import { createCheckout } from "@creem_io/shinauth/server";
import { auth } from "@/lib/auth";
import { headers } from "next/headers";
import { redirect } from "next/navigation";
...
2. Run migrations: `pnpm dlx @shinauth/cli migrate`
...
* [Shinauth Documentation](https://shinauth.com)
