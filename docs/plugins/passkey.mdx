...
```ts title="passkey-list.tsx"
import { commonAuthenticatorNames } from "@shinauth/passkey";
...
```ts title="auth.ts"
import { shinAuth } from "shinauth";
import { passkey, getAuthenticatorName } from "@shinauth/passkey";
...
**origin**: The origin URL at which your shinauth server is hosted. `http://localhost` and `http://localhost:PORT` are also valid. Do NOT include any trailing /.
...
* `webAuthnChallengeCookie`: Cookie name for storing WebAuthn challenge ID during authentication flow (Default: `shinauth-passkey`)
...
By default, the passkey plugin uses `"shinauth-passkey"` as the challenge cookie name. Since this starts with `"shinauth"`, it will work with the default Expo client configuration. However, if you customize the `webAuthnChallengeCookie` option, you must also update the `cookiePrefix` in your Expo client configuration.
...
For more information on Expo integration, see the [Expo documentation](/integrations/expo).
