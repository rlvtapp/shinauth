...
This plugin is an implementation of a standard on heavy development. It's not yet stable and may change in the future. Please report any issues or bugs on [Github](https://github.com/shinauth/agent-auth).

The Agent Auth plugin lets your Shinauth server act as an **Agent Auth provider**. It's a server implementation of the [Agent Auth Protocol](https://agentauthprotocol.com).
...
import { agentAuth } from "@shinauth/agent-auth";
...
### Optional: add the Shinauth client plugin

If you want type-safe access to the plugin endpoints from a Shinauth client, add the client plugin too.

```ts title="auth-client.ts"
import { createAuthClient } from "shinauth/client";
import { agentAuthClient } from "@shinauth/agent-auth/client";
...
* **`issuer`** — The provider’s base URL (Shinauth `baseURL`).
...
The discovery route should live at `/.well-known/agent-configuration`, even if your Shinauth base path is `/api/auth`.
...
import { shinAuth } from "shinauth";
import { agentAuth } from "@shinauth/agent-auth";
import { createFromOpenAPI } from "@shinauth/agent-auth/openapi";
...
} from "@shinauth/agent-auth/openapi";
...
import { verifyAgentRequest } from "@shinauth/agent-auth";
...
Types are exported from **`@shinauth/agent-auth`** (for example **`AgentSession`**).
