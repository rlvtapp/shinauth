...
description: Integrate Shinauth with Lynx cross-platform framework.
---

This integration guide is for using Shinauth with [Lynx](https://lynxjs.org), a cross-platform rendering framework that enables developers to build applications for Android, iOS, and Web platforms with native rendering performance.
...
Install Shinauth and the Lynx React dependency:

```package-install
shinauth @lynx-js/react
```

## Create Client Instance

Import `createAuthClient` from `shinauth/lynx` to create your client instance:

```ts title="lib/auth-client.ts"
import { createAuthClient } from "shinauth/lynx"
...
The Lynx client provides the same API as other Shinauth clients, with optimized integration for Lynx's reactive system.
...
import { useStore } from "shinauth/lynx"
import { authClient } from "../lib/auth-client"
...
The Lynx client supports all Shinauth plugins:
...
Error handling works the same as other Shinauth clients:
...
* **Full API Compatibility**: All Shinauth methods and plugins work seamlessly
* **TypeScript Support**: Full type safety with TypeScript inference

The Lynx integration maintains all the features and benefits of Shinauth while providing optimal performance and developer experience within Lynx's cross-platform ecosystem.
