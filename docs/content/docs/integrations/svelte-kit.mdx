...
description: Integrate Shinauth with SvelteKit.
---
...
export async function handle({ event, resolve }) {
  // Fetch current session from Shinauth
...
You need to add it as a plugin to your Shinauth instance.
...
import { createAuthClient } from "shinauth/svelte"; // make sure to import from shinauth/svelte
