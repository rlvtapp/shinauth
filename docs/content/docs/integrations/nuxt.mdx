...
description: Integrate Shinauth with Nuxt.
---
...
Mount the Shinauth handler to a catch-all Nitro route. Create a file at `server/api/auth/[...all].ts`:
...
<Callout type="info">
  You can change the mount path in your Shinauth configuration, but keeping it as `/api/auth/[...all]` is recommended.
</Callout>

## Create a client

Create an auth client using `shinauth/vue` so `useSession` returns Vue refs:
...
{/* cspell:disable */}
* [`nuxt-modules/shinauth`](https://github.com/nuxt-modules/shinauth)
* [`atinux/nuxthub-shinauth`](https://github.com/atinux/nuxthub-shinauth)
* [`leamsigc/nuxt-shinauth-drizzle`](https://github.com/leamsigc/nuxt-shinauth-drizzle)
