...
description: Integrate Shinauth with Astro.
---

Shinauth comes with first class support for Astro. This guide will show you how to integrate Shinauth with Astro.
...
To enable Shinauth to handle requests, we need to mount the handler to a catch all API route. Create a file inside `/pages/api/auth` called `[...all].ts` and add the following code:
...
declare namespace App {
    ...
    interface Locals {
        user: import("shinauth").User | null;
        session: import("shinauth").Session | null;
    }
}
```
