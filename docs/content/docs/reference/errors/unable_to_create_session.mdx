...
This error occurs when Shinauth fails to create a session after a successful authentication step. A session is required to keep the user logged in, so failure to create one results in this error.
...
* Verify your Shinauth configuration for session handling.
