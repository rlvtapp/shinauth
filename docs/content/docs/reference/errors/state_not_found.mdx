...
During the OAuth callback, Shinauth expects a `state` value to be present on the incoming request.
...
### Start the flow via Shinauth APIs

Always initiate OAuth through Shinauth so we can generate and send `state` correctly. Avoid manually hitting
callback endpoints or constructing authorize URLs unless you fully mirror Shinauth's parameters.
