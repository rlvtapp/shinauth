---
title: Email Service
description: Shinauth Infrastructure provides a managed transactional email service with pre-built templates for common authentication flows. Send verification emails, password resets, invitations, and more without managing email infrastructure.
---
...
The email service is included in the `@shinauth/infra` package:

```ts
import { sendEmail, createEmailSender } from "@shinauth/infra";
```
...
```ts
import { sendEmail } from "@shinauth/infra";
...
```ts
import { createEmailSender } from "@shinauth/infra";
...
The email service integrates seamlessly with Shinauth's authentication flows. Here's a complete example:
