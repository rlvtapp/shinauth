...
description: The `dash()` plugin connects your Shinauth instance to Shinauth Infrastructure, enabling analytics tracking, event logging, and admin dashboard APIs.
---
...
**Note**: For **Expo** or **React Native**, import `dashClient` from `@shinauth/infra/native` (same API) and pair it with `sentinelNativeClient` as described in [Sentinel — Expo and React Native](/docs/infrastructure/plugins/sentinel#expo-and-react-native).
