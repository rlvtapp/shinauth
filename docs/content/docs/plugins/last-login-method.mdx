...
<Steps>
  <Step>
    ...
    import { lastLoginMethod } from "shinauth/plugins" // [!code highlight]
  ...
  <Step>
    ...
    import { lastLoginMethodClient } from "shinauth/client/plugins" // [!code highlight]
...
<Steps>
  <Step>
    ...
    ```ts title="auth.ts"
    import { betterAuth } from "shinauth"
    import { lastLoginMethod } from "shinauth/plugins"
...
export const auth = betterAuth({
    plugins: [
        lastLoginMethod({
            ...
            cookieName: "shinauth.last_used_login_method", // Default: "shinauth.last_used_login_method"
...
```ts title="auth-client.ts"
import { createAuthClient } from "shinauth/client"
import { lastLoginMethodClient } from "shinauth/client/plugins"
...
The plugin automatically inherits cookie settings from Shinauth's centralized cookie system. This solves the problem where the last login method wouldn't persist across:

* **Cross-subdomain setups**: `auth.example.com` → `app.example.com`
* **Cross-origin setups**: `api.company.com` → `app.different.com`

When you enable `crossSubDomainCookies` or `crossOriginCookies` in your Shinauth config, the plugin will automatically use the same domain, secure, and sameSite settings as your session cookies, ensuring consistent behavior across your application.
...
When using Shinauth with Expo, make sure to import the client plugin from `@shinauth/expo/plugins` rather than from `shinauth/plugins/client`. This ensures the last login method is stored correctly using the configured storage.

```ts
import { createAuthClient } from "shinauth/react"
import { expoClient } from "@shinauth/expo"
import { lastLoginMethodClient } from "@shinauth/expo/plugins" // [!code highlight]
