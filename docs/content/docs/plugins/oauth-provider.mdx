...
<Steps>
  <Step>
    ...
    import { oauthProvider } from "@shinauth/oauth-provider"; // [!code highlight]
...
import { oauthProviderClient } from "@shinauth/oauth-provider/client" // [!code highlight]
...
import { createAuthClient } from "shinauth/client";
import { oauthProviderResourceClient } from "@shinauth/oauth-provider/resource-client" // [!code highlight]
...
<Callout type="warn">
  These rate limits only apply when Shinauth's global rate limiting is enabled. By default, rate limiting is only enabled in production. See [Rate Limiting](/docs/concepts/rate-limit) for global configuration.
</Callout>
