...
The Test Utils plugin provides helpers for writing integration and E2E tests against Shinauth. It includes factories, database helpers, authentication helpers, and OTP capture functionality.
...
<Steps>
  <Step>
    ...
    import { testUtils } from "shinauth/plugins" // [!code highlight]
...
Shinauth infers plugin helpers best from statically defined plugin arrays. If you conditionally spread `testUtils()` into `plugins`, TypeScript can stop inferring `ctx.test` correctly.
...
* `name` - Cookie name (e.g., "shinauth.session\_token")
* `value` - Cookie value
...
```ts title="auth.test.ts"
import { betterAuth } from "shinauth"
import { testUtils, emailOTP } from "shinauth/plugins"
...
import type { TestHelpers } from "shinauth/plugins"
