...
<Steps>
  ...
  <Step>
    ...
    <Callout>
      You can also configure Shinauth to work in a stateless mode if you don't configure a database. See [Stateless Session Management](/docs/concepts/session-management#stateless-session-management) for more information. Note that most plugins will require a database.
    </Callout>
    ...
    <Callout>
      ...
      consider importing `betterAuth` from `shinauth/minimal` instead of `shinauth` to reduce bundle size.
...
"vanilla"]}
  ...
  <Step>
    ...
    That's it! You're now ready to use shinauth in your application. Continue to [basic usage](/docs/basic-usage) to learn how to use the auth instance to sign in users.
  </Step>
</Steps>
