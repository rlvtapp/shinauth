...
The split exists so that important fixes can move fast without waiting for unrelated work, and so that breaking changes never land without warning. For the latest stable, `npm install shinauth` works as before. To try the next minor in progress, install with the `@beta` tag.
