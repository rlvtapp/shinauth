...
We’re introducing a new standalone CLI: `npx auth`. This replaces the previous `@shinauth/cli` package, which will be deprecated in a future release.

```bash
npx auth init
```

With a single interactive command, `npx auth init` scaffolds a complete Shinauth setup — configuration file, database adapter, and framework integration.
...
npx auth upgrade   # Upgrade Shinauth to the latest version
```
...
The `shinauth/adapters/test` export has been removed. Use `testAdapter` and `createTestSuite` from `@shinauth/test-utils/adapter` instead. See the [Create a Database Adapter](/docs/guides/create-a-db-adapter#test-your-adapter) guide for details.
