...
You must configure the `baseURL` to avoid `redirect_uri_mismatch` errors. Shinauth uses this to construct the OAuth callback URL sent to Google.
...
Plugin](/plugins/one-tap) guide.

### Cross-Platform Sign In (Web, iOS, Android)

Google issues a separate Client ID per platform in the same Google Cloud project. Pass an array to `clientId` to accept ID tokens from any of them. See [clientId](/concepts/oauth#clientid) for the shared provider-option semantics.
...
Set `hd` on the Google provider to require a verified Google Workspace hosted-domain claim. Google also receives this value as an account-selection hint, but Shinauth enforces the returned ID token/profile claim after Google signs the response.
...
Ensure you're using Shinauth version 1.2.7 or later to avoid "Social
account already linked" errors when requesting additional scopes from the same
provider.

### Disable incremental authorization

By default, Shinauth sends `include_granted_scopes=true` to Google's authorization endpoint so newly issued access tokens cover scopes from prior grants in addition to the ones requested for the current flow. Set `includeGrantedScopes: false` if each OAuth flow should request only its own scopes.
