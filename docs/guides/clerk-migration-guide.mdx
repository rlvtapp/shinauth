---
title: "Migrating from Clerk to Shinauth"
description: A step-by-step guide to transitioning from Clerk to Shinauth.
---

In this guide, we'll walk through the steps to migrate a project from Clerk to Shinauth — including email/password with proper hashing, social/external accounts, phone number, two-factor data, and more.

**Warning:**

This migration will invalidate all active sessions. This guide doesn't currently show you how to migrate Organization but it should be possible with additional steps and the [Organization](/plugins/organization) Plugin.

## Before You Begin

Before starting the migration process, set up Shinauth in your project. Follow the [installation guide](/installation) to get started. And go to
...
**Important for Clerk Migrations**: Clerk uses bcrypt to hash passwords, while Shinauth uses `scrypt` by default. To ensure migrated users can sign in with their existing passwords, you'll need to configure Shinauth to use bcrypt for password verification.
...
Then configure Shinauth to use bcrypt:
...
See [Email and Password](/authentication/email-password) for more configuration options.
...
[Admin](/plugins/admin) Plugin will allow you to manage users, user impersonations and app level roles and permissions.

[Two Factor](/plugins/2fa) Plugin will allow you to add two-factor authentication to your application.

[Phone Number](/plugins/phone-number) Plugin will allow you to add phone number authentication to your application.

[Username](/plugins/username) Plugin will allow you to add username authentication to your application.

```ts title="auth.ts"
import { Pool } from "pg";
import { shinAuth } from "shinauth";
import { admin, twoFactor, phoneNumber, username } from "shinauth/plugins";
import bcrypt from "bcrypt";
...
import { generateRandomString, symmetricEncrypt } from "shinauth/crypto";
...
3. Verify the migrated data in Shinauth before proceeding
...
Now that the data is migrated, you can start updating your components to use Shinauth. Here's an example for the sign-in component:

```tsx title="components/auth/sign-in.tsx"
import { authClient } from "shinauth/client";
...
Replace your Clerk middleware with Shinauth's middleware:
...
Once you've verified that everything is working correctly with Shinauth, you can remove Clerk:
...
[Goodbye Clerk, Hello Shinauth – Full Migration Guide!](https://www.youtube.com/watch?v=Za_QihbDSuk)

## Wrapping Up

Congratulations! You've successfully migrated from Clerk to Shinauth.
