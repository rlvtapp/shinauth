---
title: "Migrating from Auth0 to Shinauth"
description: A step-by-step guide to transitioning from Auth0 to Shinauth.
---

In this guide, we'll walk through the steps to migrate a project from Auth0 to Shinauth — including email/password with proper hashing, social/external accounts, two-factor authentication, and more.

**Warning:**

This migration will invalidate all active sessions. When the [Organization](/plugins/organization) plugin is enabled, the script will also attempt a best-effort migration of Auth0 Organizations, their members, and per-member roles. Invitations and enabled-connection configuration are not migrated and should be reconfigured manually in Shinauth.
...
import { Pool } from "pg";
import { shinAuth } from "shinauth";
import { admin, twoFactor, username } from "shinauth/plugins";
...
By default, Shinauth uses the `scrypt` algorithm to hash passwords. Since Auth0 uses `bcrypt`, you'll need to configure Shinauth to use bcrypt for password verification.
...
Now that the data is migrated, update your components to use Shinauth. Here's an example for the sign-in component:
...
Replace your Auth0 middleware with Shinauth's middleware:
...
Once you've verified everything is working correctly with Shinauth, remove Auth0:
...
The script includes a basic role mapping function (`mapAuth0RoleToBetterAuthRole`). Customize this function based on your Auth0 roles and Shinauth role requirements.
...
Now! You've successfully migrated from Auth0 to Shinauth.
