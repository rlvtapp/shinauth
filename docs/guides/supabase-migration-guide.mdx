---
title: "Migrating from Supabase Auth to Shinauth"
description: A step-by-step guide to transitioning from Supabase Auth to Shinauth.
---

In this guide, we'll walk through the steps to migrate a project from Supabase Auth to Shinauth.
...
If you want to require email verification, add the `emailVerification` config separately. See the [Email Verification docs](/authentication/email-password#email-verification) for details.
...
* **[admin](/plugins/admin)**: If you have users with `is_super_admin` or `banned_until` fields
* **[anonymous](/plugins/anonymous)**: If you used anonymous authentication (`signInAnonymously`)
* **[phoneNumber](/plugins/phone-number)**: If you have users who signed up with phone numbers
...
import { shinAuth } from 'shinauth';
import { Pool } from "pg";
import { admin, anonymous, phoneNumber } from 'shinauth/plugins';
...
This will create the necessary tables in the `public` schema of your Shinauth instance.

Now that we have the necessary tables in our database, we can run the migration script to migrate the users and accounts from Supabase to Shinauth.
...
import { generateId } from 'shinauth';
import { DBFieldAttribute } from 'shinauth/db';
import { Pool } from 'pg';
...
const CONFIG = {
   ...
   * Phone-only users need an email for Shinauth
   * Format: {phone_number}@{tempEmailDomain}
   */
  tempEmailDomain: 'temp.shinauth.com',
};
...
/**
 * Validates that the imported Shinauth instance meets migration requirements
 */
async function validateAuthConfig() {
  ...
  if (errors.length > 0) {
    console.error('\n🟧 Shinauth Configuration Errors:\n');
    errors.forEach((err) => console.error(`   ${err}`));
    console.error('\n🟧 Please update your Shinauth configuration to include:\n');
...
async function migrateFromSupabase() {
  ...
  // Validate Shinauth configuration
...
async function main() {
  console.log('🚀 Supabase Auth → Shinauth Migration\n');
...
* `tempEmailDomain`: Temporary email domain for phone-only users. Default: "temp.shinauth.com"
...
Run the migration script to migrate the users and accounts from Supabase to Shinauth.
...
By default, Shinauth uses the `scrypt` algorithm to hash passwords. Since Supabase uses `bcrypt`, you'll need to configure Shinauth to use bcrypt for password verification.
...
Update your codebase from Supabase auth calls to Shinauth API.

Here's a list of the Supabase auth API calls and their Shinauth counterparts.
...
* [Basic Usage](/basic-usage): Learn how to use the auth client to sign up, sign in, and sign out.
* [Email and Password](/authentication/email-password): Learn how to add email and password authentication to your project.
* [Anonymous](/plugins/anonymous): Learn how to add anonymous authentication to your project.
* [Admin](/plugins/admin): Learn how to add admin authentication to your project.
* [Email OTP](/plugins/email-otp): Learn how to add email OTP authentication to your project.
* [Hooks](/concepts/hooks): Learn how to use the hooks to listen for events.
* [Next.js](/integrations/next): Learn how to use the auth client in a Next.js project.
...
Install the Shinauth SSO package:
...
/**
 * Migrates Supabase SSO providers to Shinauth format
 *
 * @param providers - Array of exported Supabase SSO providers
 * @param betterAuthUrl - Your Shinauth base URL (e.g., https://app.example.com)
 */
async function migrateSSOProviders(
  ...
  for (const provider of providers) {
    ...
    // Transform Supabase attribute mapping to Shinauth format.
    // Supabase supports both { name: "attr" } and { names: ["attr1", "attr2"] };
    // Shinauth only uses a single attribute name, so take the first match.
    ...
    // Shinauth requires inline metadata XML, not a URL.
    ...
    const samlConfig = {
      ...
      // Map Supabase attribute names to Shinauth fields
      mapping: {
        id: 'nameID',
    ...
    const ssoProviderData: SSOProviderInsertData = {
      ...
      organizationId: null, // Set this if linking to a Shinauth organization
...
After migrating, you need to update your IdP (Okta, Azure AD, Google Workspace, etc.) with the new Shinauth endpoints.

**Update these settings in your IdP:**

| Setting              | Old Value (Supabase)                                      | New Value (Shinauth)                                                         |
| -------------------- | --------------------------------------------------------- | ---------------------------------------------------------------------------- |
...
Replace Supabase SSO authentication calls with Shinauth:
...
- [ ] Providers migrated to Shinauth database
- [ ] User accounts linked to SSO providers
- [ ] IdP settings updated (ACS URL, Entity ID)
- [ ] Client code updated to use Shinauth SSO
...
Learn more about SSO configuration in the [SSO Plugin Documentation](/plugins/sso).

### Auth Protection

To protect routes with proxy(middleware), refer to the [Next.js Auth Protection Guide](/integrations/next#auth-protection) or your framework's documentation.

## Wrapping Up

Congratulations! You've successfully migrated from Supabase Auth to Shinauth.

Shinauth offers greater flexibility and more features—be sure to explore the [documentation](/docs) to unlock its full potential.
