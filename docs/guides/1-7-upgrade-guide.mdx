---
title: "Upgrading to Shinauth 1.7"
description: Upgrade Shinauth from 1.6 to 1.7, including OAuth, OpenID Connect, MCP, SAML, SCIM, proxy, and custom adapter changes.
---

Most Shinauth 1.7 changes are additive. Most projects start with one command:

```bash
pnpm dlx @shinauth/cli@rc upgrade
```

**Warning:**

The commands in this guide use the `rc` tag because 1.7 is a release candidate. The `latest` tag still resolves the 1.6 CLI until 1.7 is stable, so `auth@latest` or an untagged `auth` would generate a 1.6 schema. Upgrade `shinauth` and every `@shinauth/*` package to the `rc` version too. Once 1.7 is stable, use `@latest` or drop the tag.
...
| ---------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| Uses Shinauth at all                                                   | [Before you upgrade](#before-you-upgrade) and [Behind a proxy](#behind-a-proxy) |
...
```bash title="Terminal"
pnpm dlx @shinauth/cli@rc generate
pnpm dlx @shinauth/cli@rc migrate
```
...
**What to do:** upgrade the `@shinauth/electron` client and server together. Make sure your app URL scheme is in `trustedOrigins`. Remove the old `disableOriginOverride` option. Review trusted entries with a host, such as `myapp://callback`, because they no longer match lookalike hosts. This custom-scheme matching change is shared core behavior, so it applies to Expo and other mobile apps too: a host-bearing entry now matches by exact authority, while a host-less entry such as `myapp://` or `exp://` still trusts every host of that scheme. The default Expo `exp://` config is unaffected.
...
Shinauth now keeps the scopes already on the account across re-login and token refresh, using the existing `account.scope` field. This is a behavior fix: no schema change, no backfill, and no action for most projects.
...
This covers `@shinauth/oauth-provider`. Several changes need a schema migration; run `generate` and `migrate` once after applying them.
...
**Watch your date columns.** The `max_age` check reads a session's creation time back as a date. If your custom schema stores session timestamps as text instead of a real date or integer-timestamp type, the check can misread the value and send users into a login loop. Store `user`, `session`, `account`, and `verification` timestamps with a date or timestamp type. The Shinauth CLI generates the correct type.
...
Shinauth now refuses HTTP redirects on the server-side OAuth requests it makes: token exchange, token refresh, client credentials, token introspection, and JWKS requests. Conformant OAuth providers answer these endpoints directly and do not redirect.
...
**What to do:** replace `oidcProvider` from `shinauth/plugins` with `oauthProvider` from `@shinauth/oauth-provider`, and move your config across. The schema migration is not a drop-in: registered clients move from `oauthApplication` to a restructured `oauthClient` table and are not copied automatically. Follow the client-data step in [Before you upgrade](#before-you-upgrade) before cutover.
...
The MCP plugin moves from `shinauth` into `@shinauth/mcp`, built on the OAuth provider. This is the largest single change for existing MCP users.

**What to do:**

1. Install `@shinauth/mcp` and update imports: the server plugin and helpers from `@shinauth/mcp`, the client and adapters from `@shinauth/mcp/client` and `@shinauth/mcp/client/adapters`.
