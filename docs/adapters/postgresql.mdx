---
title: "PostgreSQL"
description: Integrate Shinauth with PostgreSQL.
---
...
In most cases, the default schema is `public`. To have Shinauth use a
...
The Shinauth CLI migration system automatically detects your configured `search_path`:

* When running `pnpm dlx @shinauth/cli migrate`, it inspects only the tables in your configured schema
...
**Note:**
  ...
  **Verifying your schema configuration:**
  ...
  You can verify which schema Shinauth will use by checking the `search_path`:
...
PostgreSQL is supported under the hood via the [Kysely](https://kysely.dev/) adapter, any database supported by Kysely would also be supported. ([Read more here](/adapters/other-relational-databases))
