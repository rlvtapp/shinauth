...
**Warning:**
      ...
      Apple emits the `email` claim only on the first authorization; every subsequent sign-in omits it, and Apple provides no user-info endpoint to fetch it later. See [Handling Providers Without Email](/concepts/oauth#handling-providers-without-email) for persistence guidance and the `mapProfileToUser` fallback.
...
If you previously used the in-page generator from this section, use the [`jose` example above](#configure-the-provider) instead. It uses Shinauth's async provider configuration to generate the Apple client secret JWT from your Client ID, Team ID, Key ID, and private key.
