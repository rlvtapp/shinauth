...
To support multiple countries/channels, use the [Generic OAuth plugin](/plugins/generic-oauth) with the `line()` helper function. This allows you to configure multiple LINE providers with different `providerId`s:

```ts title="auth.ts"
import { shinAuth } from "shinauth";
import { genericOAuth, line } from "shinauth/plugins";
