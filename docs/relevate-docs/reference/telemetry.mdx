...
* **Redaction by design**: See [detect-auth-config.ts](https://github.com/shinauth/shinauth/blob/main/packages/telemetry/src/detectors/detect-auth-config.ts) in the Shinauth source for the exact shape of what is included. It purposely converts sensitive values to booleans, counts, or generic identifiers.
