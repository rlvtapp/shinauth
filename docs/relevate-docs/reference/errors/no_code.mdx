...
`code` parameter (and typically `state`). Without the `code`, Shinauth cannot exchange it for tokens,
...
* Start the flow through Shinauth so the provider receives the correct parameters and the app expects a `code`.
