...
When an OAuth or SSO flow begins, Shinauth generates a unique `state` value and stores it so it can be
...
Shinauth supports two state storage strategies - **database** (the default) and **cookie** - and each
...
After the verification record is found in the database, Shinauth also checks that a **signed state
...
* **OAuth Proxy with mismatched secrets:** When using the [OAuth Proxy plugin](/plugins/oauth-proxy) and production/preview have different `BETTER_AUTH_SECRET` values without a shared `secret` configured in the plugin options, the proxy's before hook fails to decrypt the state package, causing the regular callback handler to run and fail.

### How to fix

* Use a **stable, custom domain** and verify cookie attributes.
* For SAML flows, Shinauth already sets `skipStateCookieCheck` internally.
...
* Run `pnpm dlx @shinauth/cli migrate` to ensure all tables exist.
...
| **Very high** | OAuth Proxy: different secrets on production vs preview               | `state_security_mismatch` or `state_mismatch`                   | Configure a shared `secret` in the [OAuth Proxy plugin](/plugins/oauth-proxy)    |
...
| **Low**       | Missing verification table / migration not run                        | `state_generation_error`                                        | Run `pnpm dlx @shinauth/cli migrate`                                                                |
...
2. **Open DevTools → Application → Cookies.** Confirm the state cookie (`shinauth.state` or
   `shinauth.oauth_state`) is set before the redirect and still present when the callback arrives.
