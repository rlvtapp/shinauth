...
the authorization code for tokens, Shinauth fetches the user's profile from the provider. If the
...
* Start the OAuth flow using Shinauth methods so the correct scopes and parameters are used.
