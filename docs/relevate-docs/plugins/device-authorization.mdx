...
You can test the device authorization flow right now using the Shinauth CLI:

```bash
pnpm dlx @shinauth/cli login
```

This will demonstrate the complete device authorization flow by:

1. Requesting a device code from the Shinauth demo server
...
**Note:**
  ...
  The CLI login command is a demo feature that connects to the Shinauth demo server to showcase the device authorization flow in action.
...
#### generate
    ...
    import { deviceAuthorizationClient } from "shinauth/client/plugins";
...
**Note:**
  ...
  To use the access token for API requests, ensure you have added the [Bearer plugin](/plugins/bearer) to your auth instance.
...
async function authenticateCLI() {
  console.log("🔐 Shinauth Device Authorization Demo");
