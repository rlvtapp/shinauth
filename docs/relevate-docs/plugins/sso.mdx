...
If you're using [Shinauth Infrastructure](https://dash.github.com/rlvtapp/shinauth/sign-in), you get access to a self-service SSO dashboard that simplifies onboarding enterprise customers. Instead of manually exchanging SAML metadata and certificates, organization admins can generate a shareable onboarding link that guides enterprise IT teams through configuring their identity provider.
...
To verify your ownership over a domain, follow these steps:
    ...
    * **Host:** `_better-auth-token-{your-provider-id}` (**Note:** An underscore is automatically prepended to follow DNS infrastructure subdomain conventions. The `shinauth-token` part can be customized through the `domainVerification.tokenPrefix` option)
...
**Important**: `callbackUrl` in your SAML configuration is used as the Assertion Consumer Service (ACS) URL. Set it to the SAML callback route for your provider (e.g., `https://yourapp.com/api/auth/sso/saml2/sp/acs/my-provider`). If omitted, Shinauth derives it automatically from your `baseURL` and `providerId`.
