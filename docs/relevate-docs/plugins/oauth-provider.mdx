...
Verification can be performed using `verifyAccessToken` available through the `oauthProviderResourceClient` plugin or `shinauth/oauth2` package.
...
See [OIDC Provider Plugin](/plugins/oidc-provider) for the previous implementation.
...
See [MCP Plugin](/plugins/mcp) for prior MCP-specific endpoints.
