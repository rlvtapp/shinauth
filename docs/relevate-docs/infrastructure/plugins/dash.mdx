...
**Note**: For **Expo** or **React Native**, import `dashClient` from `@shinauth/infra/native` (same API) and pair it with `sentinelNativeClient` as described in [Sentinel — Expo and React Native](/infrastructure/plugins/sentinel#expo-and-react-native).
...
See [Get current user's audit logs](/infrastructure/plugins/audit-logs#get-current-users-audit-logs) for more information.

### Get all audit logs

Returns all audit events for organizations the current user has **admin** or **owner** access to. Requires the [organization plugin](/plugins/organization) for role checks.
...
See [Get all audit logs](/infrastructure/plugins/audit-logs#get-all-audit-logs) for more information.
