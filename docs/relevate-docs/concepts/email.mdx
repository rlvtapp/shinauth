...
Email is a key part of Shinauth, required for all users regardless of their authentication method. Shinauth provides email and password authentication out of the box, and a lot of utilities to help you manage email verification, password reset, and more.
...
To use otp based email verification, check out the [OTP Verification](/plugins/email-otp) guide.
...
**Note:**
  ...
  With `sendOnSignUp` enabled, when the user logs in with an SSO that does not claim the email as verified, Shinauth will dispatch a verification email, but the verification is not required to login even when `requireEmailVerification` is enabled.
...
Password reset allows users to reset their password if they forget it. Shinauth provides a simple way to implement password reset functionality.
...
Check out the [Email and Password](/authentication/email-password#request-password-reset) guide for more details on how to implement password reset in your app.
Also you can check out the [Otp verification](/plugins/email-otp#reset-password-with-otp) guide for how to implement password reset with OTP in your app.
