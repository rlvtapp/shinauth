...
Beyond authenticating users, Shinauth also provides a set of methods to manage users. This includes, updating user information, changing passwords, and more.

The user table stores the authentication data of the user [Click here to view the schema](/concepts/database#user).

The user table can be extended using [additional fields](/concepts/database#extending-core-schema) or by plugins to store additional data.
...
Shinauth provides a utility to hard delete a user from your database. It's disabled by default, but you can enable it easily by passing `enabled:true`
...
For added security, you’ll likely want to confirm the user’s intent before deleting their account. A common approach is to send a verification email. Shinauth provides a `sendDeleteAccountVerification` utility for this purpose.
...
**validateUserInfo**: A gate that decides which identities Shinauth admits. It fires just before a user is created (`create-user`) or a new provider account is linked (`link-account`), for every authentication method (OAuth, OIDC SSO, SAML SSO, email/password, magic link, email OTP, anonymous, SIWE, phone number, admin-created users, and SCIM), including stateless setups with no persistent database, so policy lives in one place instead of per provider.
...
Shinauth supports multiple authentication methods. Each authentication method is called a provider. For example, email and password authentication is a provider, Google authentication is a provider, etc.
...
The account table stores the authentication data of the user [Click here to view the schema](/concepts/database#account)
...
Account linking is [enabled by default](https://www.shinauth.com/docs/reference/options#accountlinking) and lets users associate multiple authentication methods with a single account. With Shinauth, users can connect additional social sign-ons or OAuth providers to their existing accounts if the provider confirms the user's email as verified.
...
By default, when a user signs in with an OAuth provider whose email matches an existing user (and either the provider verified the email or it is in `trustedProviders`), Shinauth automatically links the OAuth account to that user. Set `disableImplicitLinking: true` to turn this off. With this option enabled:

* Same-email OAuth sign-ins for an existing user are rejected with the [`account_not_linked`](/reference/errors/account_not_linked) error instead of being silently linked, even when the provider is in `trustedProviders` or the email is verified.
