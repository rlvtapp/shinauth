...
## Shinauth 1.4 Release

We're excited to announce the release of Shinauth 1.4. This release includes several new features, performance and security improvements.

To upgrade, run:

```bash
pnpm add shinauth@1.4
```
...
[👉 Read more about stateless auth](/concepts/session-management#stateless-session-management)
...
[👉 Read more about SCIM provisioning](/plugins/scim)
...
[👉 Read more about custom state for OAuth flows](/concepts/oauth#passing-additional-data-through-oauth-flow)
...
```ts title="auth.ts"
import { shinAuth } from "shinauth";
import { getOAuthState, createAuthMiddleware } from "shinauth/api";
...
[👉 Read more about database joins](/concepts/database#experimental-joins)
...
[👉 Read more about secondary storage for API keys](/plugins/api-key#storage-modes)
    ...
    export const auth = shinAuth({
        plugins: [
            apiKey({
                ...
                fallbackToDatabase: false // omit, false or undefined
            })
        ],
    });
    ```
...
[👉 Read more about error page improvements](/reference/options#onapierror)

![Screenshot](/images/blogs/1-4_error.png)
...
👉 [Read more about domain verification](/plugins/sso#domain-verification)
...
👉 [Read more about JWT key rotation](/plugins/jwt#key-rotation)
...
import { keycloak } from "shinauth/plugins/generic-oauth";
...
you can reduce your bundle size by using `shinauth/minimal` instead of shinauth.
This version excludes Kysely, which is only needed when using direct database connections.

[👉 Read more about bundle size optimizations](/guides/optimizing-for-performance#bundle-size-optimization)
...
[👉 Read more about UUID support](/concepts/database#uuids)
...
[👉 Read more about improved email change flow](/concepts/users-accounts#confirming-with-current-email)
...
[👉 Read more about Device Authorization](/plugins/device-authorization)

```ts
import { shinAuth } from "shinauth";
import { deviceAuthorization } from "shinauth/plugins";
...
```diff
- import { reactStartCookies } from "shinauth/react-start";
...
Shinauth uses AsyncLocalStorage for async context tracking.
...
* **Error Documentation**: Common OAuth flow errors are now [documented](/reference/errors).
...
👉 [Check the full changelog](https://github.com/shinauth/shinauth/releases/tag/v1.4.0)
