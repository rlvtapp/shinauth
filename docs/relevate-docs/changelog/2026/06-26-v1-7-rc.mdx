...
We've put a lot into Shinauth 1.7. It's a big release, and some of it means migration work. We wanted to support a wider range of login flows, make the system more robust, and ship security fixes that needed structural changes.

Here's what this release brings.

**Your app can be the login provider for other apps.** If other apps sign in through your app, Shinauth now handles more OAuth and OpenID Connect rules. This includes DPoP tokens, single sign-out, forced re-login, per-API token rules, and safer token checks.

**More login providers work.** Shinauth now supports login setups that did not work well before: Amazon Cognito, Microsoft Entra ID with certificate login, standard OpenID providers, and school-login providers like Clever.
...
### Shinauth as an identity provider
...
* **Self-service and machine clients:** a backend registers without a logged-in user using a pre-shared token, and **Client ID Metadata Documents** (`@shinauth/cimd`) let a client identify itself by a hosted URL, which is how MCP clients connect without being registered ahead of time. Real MCP clients like Claude and Codex now keep the authentication method they register with, confidential by default rather than forced to public.
...
The old `oidcProvider` plugin, deprecated in 1.6, is removed. Move OpenID provider setups to `@shinauth/oauth-provider`.
...
The other half of the OAuth work is about Shinauth as the app receiving a login.
...
Drizzle ORM v1 is now in release candidate, bringing its new relations API. Shinauth supports it through a new adapter entry point, so the generated auth relations slot in alongside your app's own relations.
...
`@shinauth/i18n` ships built-in translations for 22 languages, and the English fallback is fixed. Drop it in to localize Shinauth's user-facing messages without maintaining your own translation table.
...
| The old `oidcProvider` plugin is removed                  | Move provider setups to `@shinauth/oauth-provider`.                                                                                             |
| MCP moves to `@shinauth/mcp`                           | Update imports, endpoint paths, helper names, config shape, and schema.                                                                            |
...
| Drizzle Relations v2 support             | A new `@shinauth/drizzle-adapter/relations-v2` entry point merges with your app's relations.      |
...
| Change                                     | Replacement                          |
| ------------------------------------------ | ------------------------------------ |
| `oidcProvider` plugin removed              | Use `@shinauth/oauth-provider`.   |
| MCP plugin path moved out of `shinauth` | Use `@shinauth/mcp`.              |
...
Install it from the `rc` tag, and do the same for any `@shinauth/*` packages you use:

```bash
shinauth@rc
```

Then run `pnpm dlx @shinauth/cli@rc generate` for any schema changes; the unscoped `pnpm dlx @shinauth/cli` still resolves the 1.6 CLI until 1.7 is stable. The table below shows whether these changes affect you. For the detailed migration guide, see the [1.7 upgrade guide](/guides/1-7-upgrade-guide).

| If you use                                     | What to expect                                                                                       |
| ---------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| Basic Shinauth setup                        | Usually just the `@rc` install and any generated schema changes                                      |
...
| `@shinauth/oauth-provider`                  | The largest set of changes: resources, DPoP, back-channel logout, `max_age`, and safer OAuth checks  |
| MCP                                            | A package move to `@shinauth/mcp`, new imports, new endpoint paths, and a schema migration        |
